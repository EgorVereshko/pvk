  ...
  const handleSaveChecklist = async () => {
    try {
      ...
      const checklistResponse = await api.post('/api/checklist/save/', {
        ...
        qualities: tableData.qualities // Передаем все компетенции
      });
                  ...
                  <thead>
                    <tr>
                      ...
                      {[0, 1, 2, 3, 4].map(index => (
                        <th key={index}>
                          <select
                            value={tableData.students[index] || ''}
                            onChange={(e) => handleStudentSelect(index, e.target.value)}
                            className="student-select"
                          >
                            <option value="">Выберите студента {index + 1}</option>
                    ...
                    {tableData.qualities.map((quality, qIndex) => (
                      <tr key={qIndex}>
                        ...
                        {[0, 1, 2, 3, 4].map(sIndex => (
                          <td key={sIndex}>
                            <select
                              value={tableData.scores[qIndex]?.[sIndex] || ''}
